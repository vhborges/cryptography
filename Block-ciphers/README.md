...
The PCBC mode is not vulnerable to this attack since it uses both the plaintext and the ciphertext of the previous block to help decrypt the next block, so any change to any byte of any block will propagate to all the next blocks and render the ciphertext useless. My code couldn't even decrypt the resulting ciphertext because it corrupted even the padding, so that almost all bytes are discarded in the decryption process.
