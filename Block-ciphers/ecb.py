...
def encripta_bloco(plainblock: bytes, chaves: list) -> bytes:
    cipherblock = feistel_network(plainblock, chaves)
    return cipherblock

def decripta_bloco(cipherblock: bytes, chaves: list) -> bytes:
    plainblock = feistel_network(cipherblock, chaves)
    return plainblock
...
def encripta(plaintext: str, chave: bytes, tam_bloco: int, qtd_rounds: int) -> bytes:
    ...
    ciphertext = bytes()
    for bloco in blocos:
        cipherblock = encripta_bloco(bloco, chaves)
...
def decripta(ciphertext: bytes, chave: bytes, tam_bloco: int, qtd_rounds: int) -> Union[str, bytes]:
    ...
    plaintext = bytes()
    for bloco in blocos:
        plainblock = decripta_bloco(bloco, chaves)
...
cipher = encripta('From:Lucas\nTo:Lucas\nContent:criptografias de ECB sao previsiveis', b'arroz', 10, 10)
print(cipher, end="\n\n")
...
cipher = encripta('From:Lucas\nTo:Lucas\nContent:criptografias de EFB sao previsiveis', b'arroz', 10, 10)
print(cipher, end="\n\n")
print(list(cipher), end="\n\n")
